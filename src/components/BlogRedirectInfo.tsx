 ...
 export default function BlogRedirectInfo(prop: IProps) {
   ...
   function newPostUrl(paths: string[]): string {
     if (isValidPostUrl(paths)) {
       return `https://codefoundry.dev/${paths.join('/')}`;
     }
     return `https://codefoundry.dev/search?q=${paths.join(',')}&max-results=20&by-date=true`
   }
 
   function newSearch(keyword: string) {
     return `https://codefoundry.dev/search?q=${keyword}&max-results=20&by-date=true`
   }
     ...
     <>
       <div className={styles['container']}>
         <div className={styles['big-box']}>
           ...
           <p className={styles['big-box-p']}><code className={styles['code']}>https://gauravbytes.com</code> is moving to <a className={styles['big-box-a']} href="https://codefoundry.dev"><code className={styles['code']}>https://codefoundry.dev</code></a></p>
